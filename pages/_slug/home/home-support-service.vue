<template>
  <div class="max-w-6xl px-3 mx-auto">
    <home-service>
      <template #category>
        <home-header-category title="HỖ TRỢ DỊCH VỤ" :view-all="false" />
      </template>

      <template #content>
        <div
          class="grid grid-cols-1 sm:grid-cols-2 gap-5"
          :class="{ 'sm:grid-cols-3': GetLocation.slug === 'admin' }"
        >
          <template v-for="item in entries">
            <div
              v-if="
                item.title != 'Góp ý dịch vụ' ||
                (item.title === 'Góp ý dịch vụ' && GetLocation.slug === 'admin')
              "
              :key="item.title"
      ...
      entries: [
        {
          ...
          route: 'web.slug.support-services.freq-asked-questions',
        },
        {
          title: 'Góp ý dịch vụ',
          image: require('@/assets/images/feedback.png'),
          route: 'web.slug.support-services.feedback',
        },
        {
          title: 'Video hướng dẫn',
          image: require('@/assets/images/video-tutorial.png'),
          route: 'web.slug.support-services.video-tutorials',
        },
      ],
    }
  },
