<template>
	<div
		v-if="entries?.length"
		class="max-w-6xl px-3 mx-auto overflow-hidden font-inter"
	>
		<HomeService>
			<template #category>
				<HomeHeaderCategory
					title="THIẾT BỊ THÔNG MINH"
					@viewAll="$router.push({ name: 'web.slug.smart-devices' })"
				/>
			</template>
			...
			<template v-if="entries?.length" #content>
				<VueSlickCarousel v-bind="settings">
					...
					>
						<div
							class="border-[1px] border-solid border-[#ccc] overflow-hidden rounded-md"
						>
							...
							<div class="p-2">
								...
								<div
									class="text-[#4f4f4f] text-[18px] font-semibold"
								>
